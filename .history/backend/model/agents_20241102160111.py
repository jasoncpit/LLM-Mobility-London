...
class agent:
    def __init__(self,llm):
        ...
        self.weekly_scheduler = ChatPromptTemplate.from_messages([
            ("system", SCHEDULER_SYSTEM_PROMPT)]) | llm.with_structured_output(WeeklySchedule)

        self.weekly_planner = ChatPromptTemplate.from_messages([
        ('system' , PLANNER_SYSTEM_PROMPT)]) | llm.with_structured_output(WeeklyPlanner) 
...
if __name__ == "__main__":
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.5)
    agent = agent(llm)
    result = agent.weekly_planner.invoke(
    ...
    example_schedule  = result.days[1].summary 
    daily_planner = agent.weekly_scheduler.invoke(
        {'user_description': "A 30 year old married software engineer lives in Stratford and works in "
        "West Kensignton. He loves to travel and explore new places. He works from home three times a week.",
        'daily_agenda': monday_schedule}
    )
    print(daily_planner)

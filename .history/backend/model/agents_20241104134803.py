...
class agent:
    ...
    def create_weekly_scheduler(self):
       ...
       return ChatPromptTemplate.from_messages([
            ("system", SCHEDULER_SYSTEM_PROMPT)]) | self.llm.with_structured_output(WeeklySchedule)

    def create_weekly_planner(self):
        return ChatPromptTemplate.from_messages([
        ('system' , PLANNER_SYSTEM_PROMPT)]) | self.llm.with_structured_output(WeeklyPlanner) 
...
if __name__ == "__main__":
    ...
    daily_planner = agent.create_weekly_scheduler.invoke(
        {'user_description': "A 30 year old married software engineer lives in Stratford and works in "
        "West Kensignton. He loves to travel and explore new places. He works from home three times a week.",

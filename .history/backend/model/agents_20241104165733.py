...
from output_classes import WeeklySummary,DailySummary 
import dotenv
import os 
class agent:
    def __init__(self,llm):
        self.llm = llm    
    def create_weekly_scheduler(self):
        # Weekly Schedule LLM planner -> given user description, generate weekly planner 
       return ChatPromptTemplate.from_messages([
            ("system", SCHEDULER_SYSTEM_PROMPT)]) | self.llm.with_structured_output(WeeklySummary)

    def create_weekly_planner(self):
        return ChatPromptTemplate.from_messages([
        ('system' , PLANNER_SYSTEM_PROMPT)]) | self.llm.with_structured_output(DailySummary) 

...
class GoogleAPIClient:
    ...
    def compute_daily_routes(self, daily_plan):
        ...
        for i in range(len(entries)-1):
            ...
            # 4. Same POI name
            if (current.poi_output.name == next_entry.poi_output.name or
            ...
            try:    
                ...
                current.travel_mode.value if hasattr(current.travel_mode, 'value') else 'WALK'
                )
            ...
            if route_result and 'routes' in route_result:
                ...
                routes.append({
                    ...
                    'travel_mode': current.travel_mode.value if hasattr(current.travel_mode, 'value') else 'WALK',
                    'geometry': route_line
                })
            else:
                routes.append(None)

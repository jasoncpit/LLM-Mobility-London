...
function App() {
  ...
  const getDayFromTime = (time) => {
    ...
    return days[Math.floor(time / (24 * 60)) % 7];
  };

  const formatTimeLabel = (time) => {
    const day = getDayFromTime(time);
    const hour = Math.floor((time % (24 * 60)) / 60);
  ...
  useEffect(() => {
    ...
    const loadTraces = async () => {
      ...
      // Define trace colors
      const traceColors = {
        0: [255, 0, 0],    // First trace - Red
        1: [0, 255, 0],    // Second trace - Green
        2: [0, 0, 255]     // Third trace - Blue
      };
      ...
      const allTrips = traces.flatMap((trace, traceIndex) => {
        ...
        return routes.map(route => ({
          path: route,
          color: traceColors[traceIndex] // Use trace-specific color instead of mode-based color
        }));
      });
  ...
  const layers = [
    ...
    new ScatterplotLayer({
      ...
      data: pointData.filter(d => {
        const currentDay = Math.floor(currentTime / (24 * 60)) % 7;

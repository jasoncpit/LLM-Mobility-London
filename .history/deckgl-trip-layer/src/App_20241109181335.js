...
function App() {
  ...
  useEffect(() => {
    const loadTraces = async () => {
      try {
        ...
        // Find the global min and max timestamps across all traces
        let minTime = Infinity;
        let maxTime = -Infinity;

        // Combine all traces with trace-specific colors and normalize timestamps
        const allTrips = traces.flatMap((trace, traceIndex) => {
          const routes = trace.trip_layer.routes;
          const timestamps = trace.trip_layer.timestamps || []; // Add this if you have timestamps in your data

          // Update min/max time if timestamps exist
          if (timestamps.length > 0) {
            minTime = Math.min(minTime, ...timestamps.flat());
            maxTime = Math.max(maxTime, ...timestamps.flat());
          }

          return routes.map((route, idx) => ({
            path: route,
            timestamps: timestamps[idx] || route.map((_, i) => i), // Use actual timestamps if available
        ...
        // If no actual timestamps were found, use sequential numbering
        if (minTime === Infinity) {
          minTime = 0;
          maxTime = 1000;
        }
        ...
        console.log('Processed trips:', normalizedTrips); // Debug log
    ...
    if (isPlaying) {
      const animationTimer = setInterval(() => {
        setCurrentTime(time => {
          ...
          // Reset to 0 when we reach the max time
          return nextTime > timeRange.max - timeRange.min ? 0 : nextTime;
        });
  ...
  const layers = [
    new TripsLayer({
      ...
      getPath: d => d.path,
      getTimestamps: d => d.timestamps, // Use normalized timestamps
      getColor: d => d.color,
      opacity: 0.8,
      widthMinPixels: 2,
      trailLength: 180,

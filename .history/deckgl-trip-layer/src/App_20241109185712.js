...
function App() {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      animationTimer = setInterval(() => {
        setCurrentTime(time => (time + 3) % 1000);
      }, 50);
  ...
  const layers = [
    new TripsLayer({
      ...
      currentTime,
      fadeTrail: true,
      getTimestamps: d => d.path.map((_, i) => i),
      trail: 1000
    }),
    new ScatterplotLayer({
      ...
      getColor: d => {
        ...
        return [...traceColors[d.traceIndex], 204];
      },
      getRadius: d => {
        if (selectedPoint && 
            d.position[0] === selectedPoint.position[0] && 
  ...
  const renderPlaybackControls = () => (
    <div style={{
      ...
      <div style={{
        ...
        <label style={{ fontSize: '0.9em', color: '#666' }}>Trail Length:</label>
        <input
          type="range"
          min="50"
          max="500"
          ...
          style={{ flex: 1 }}
        />
        <span style={{ fontSize: '0.9em', color: '#666', minWidth: '40px' }}>{trailLength}</span>
      </div>
    </div>
  );
                  ...
                  {activities.sort((a, b) => a.time.localeCompare(b.time)).map((activity, idx) => (
                    <div 
                      ...
                      style={{
                        ...
                        cursor: 'pointer',
                        background: selectedPoint === activity ? 'rgba(255, 255, 0, 0.1)' : 'transparent'
                      }}
                      onClick={() => {
                        setSelectedPoint(selectedPoint === activity ? null : activity);
                        if (deckRef.current) {
